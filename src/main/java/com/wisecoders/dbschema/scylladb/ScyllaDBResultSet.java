...
import com.wisecoders.dbschema.scylladb.types.ArrayImpl;
import com.wisecoders.dbschema.scylladb.types.BlobImpl;
...
import static com.wisecoders.dbschema.scylladb.DateUtil.considerTimeZone;
 ...
 * Code modifications allowed only as pull requests to the <a href="https://github.com/wise-coders/scylladb-jdbc-driver">public GIT repository</a>.
 */

public class ScyllaDBResultSet implements ResultSet {
    ...
    ScyllaDBResultSet(Statement statement, com.datastax.oss.driver.api.core.cql.ResultSet dsResultSet, boolean returnNullStrings) {
    ...
    ScyllaDBResultSet(Statement statement, com.datastax.oss.driver.api.core.cql.ResultSet dsResultSet) {
    ...
    public ResultSetMetaData getMetaData() throws SQLException {
        ...
        List<ScyllaDBResultSetMetaData.ColumnMetaData> columnMetaData = new ArrayList<>();
        for (Iterator<ColumnDefinition> itr = dsResultSet.getColumnDefinitions().iterator(); itr.hasNext(); ) {
            ColumnDefinition def = itr.next();
            columnMetaData.add(new ScyllaDBResultSetMetaData.ColumnMetaData(def.getName().toString(), def.getTable().toString(), def.getKeyspace().toString(), def.getType().toString() ));
        }
        return new ScyllaDBResultSetMetaData(columnMetaData);
    }
