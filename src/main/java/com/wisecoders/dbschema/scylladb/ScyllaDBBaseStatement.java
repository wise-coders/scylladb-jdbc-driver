 ...
 */
public abstract class ScyllaDBBaseStatement implements Statement {
    final CqlSession session;
    BatchStatementBuilder batchStatementBuilder = null;
    private boolean isClosed = false;
    ScyllaDBResultSet result;

    ScyllaDBBaseStatement(CqlSession session) {
    ...
    boolean executeInner(com.datastax.oss.driver.api.core.cql.ResultSet resultSet, boolean returnNullStrings) throws SQLException {
        try {
            result = new ScyllaDBResultSet(this, resultSet, returnNullStrings);
            if (!result.isQuery()) {
                result = null;
                return false;
            }
